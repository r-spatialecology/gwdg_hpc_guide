...
Login to the HPC and create a file called `slurm.tmpl` (same same `nano slurm.tmpl`) in your home directory and copy and paste the following in it.
...
This file is used to control how you submit jobs. It allows to pass arguments as `R` syntax from the `R` session to the LSF system as `sbatch` commands.
...
bsub <- tweak(batchtools_lsf, template = 'slurm.tmpl',
              resources = list(job.name = 'run_hpc',
                               log.file = 'run_hpc.log',
                               queue = 'mpi-short',
                               walltime = '00:30',
...
bsub <- tweak(batchtools_lsf, template = 'lsf.tmpl',       # here we tell future.batchtools where to look for the template file we created in section
              resources = list(job.name = 'run_hpc',       # name of your jobs in the SLURM scheduling system
                               log.file = 'run_hpc.log',   # logfiles
...
bsub <- tweak(batchtools_lsf, template = 'lsf.tmpl',       # here we tell future.batchtools where to look for the template file we created in section
              resources = list(job.name = 'run_hpc',       # name of your jobs in the SLURM scheduling system
                               ...
                               queue = 'mpi-short',        # set the queue you plan to use
                               walltime = '00:30',         # set a walltime for your hobs
                               processes = 1))             # block n cores on each node
...
options(
    clustermq.scheduler = "slurm",
    clustermq.template = "/home/uni08/user_name_gwdg/slurmmq.tmpl"
)
```

Then, we again create an SLURM-template as in the `future workflow` with `nano slurmmq.tmpl` and insert the following.
